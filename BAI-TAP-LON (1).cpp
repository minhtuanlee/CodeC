...
void hienthitheoCot();
	...
	void XuatN(city_st fc[],int n)
	{
           ...
           void hienthithongtinthanhpho(struct city_st cs)
              {
              	printf("||%-20d  ||%-20s  ||%-20s  ||%-20d \n",cs.Stt,cs.country,cs.year);
			  }
		...
		void timtheoquocgia(struct city_st *fc,int n)
		   {
		   	...
		   	for(i=0;i<n;i++)
				  ...
				  if(timS ==0)
				   {
				   	printf("\n Khong co quoc gia %s trong danh sach !",city_nm);
				   }
		   ...
		   void timtheonam(struct city_st *fc,int n)
		    {
			  ...
			  void hienthitheoCot()
			   ...
			   void hienthiDSSS(struct city_st *fc,int n)
				...
				/*
				void Ghidanhsachvaoteptin(struct  city_st *cs,int n)
				 {
   ...
   for (int i = 0; i < n; i++)
   {
       fwrite(&fc[i],sizeof(struct city_st),1,fp);
    }
...
void Docdanhsachsachtuteptin(struct city_st *fc , int n)
{
    int i = 0;
   FILE *fp;
   ...
   while(!feof(fp))
   {
        fread(&fc[i],sizeof(struct city_st),1,fp);
       i++;
   }
   n = i-1;
   printf("\t\nTong so thanh pho trong thu vien la:%d",n);
   fclose(fp);
   printf("\nBam enter de tiep tuc.");
   getch();
}
